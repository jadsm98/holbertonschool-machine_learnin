...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    for i, layer in enumerate(hidden_layers):
        if i == 0:
            encoder.add(keras.layers.Dense(layer, input_shape=(input_dims,), activation='relu'))
        else:
    ...
    for i in range(l, -1, -1):
        if i == l:
            decoder.add(keras.layers.Dense(hidden_layers[i], input_shape=(latent_dims,),
                                           activation='relu'))
        else:
            decoder.add(keras.layers.Dense(hidden_layers[i], activation='relu'))
