...
class Yolo:
    ...
    def preprocess_images(self, images):
        ...
        def predict(self, folder_path):
            """method"""
            images, image_path = self.load_images(folder_path)
            pimages, image_size = self.preprocess_images(images)
            predictions = []
            for i, im in enumerate(pimages):
                im = np.expand_dims(im, 0)
                output = self.model.predict(im)
                outputs = [np.squeeze(i, axis=0) for i in output]
                boxes, box_confidences, box_class_probs = \
                    self.process_outputs(outputs, image_size[i])
                filtered_boxes, box_classes, box_scores = \
                    self.filter_boxes(boxes, box_confidences, box_class_probs)
                pred_box, pred_classes, pred_scores = \
                    self.non_max_suppression(filtered_boxes, box_classes, box_scores)
                predictions.append((pred_box, pred_classes, pred_scores))
                file_name = os.path.basename(image_path[i])
                self.show_boxes(images[i], pred_box, pred_classes, pred_scores, file_name)
